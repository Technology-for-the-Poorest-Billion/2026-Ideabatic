...
## Conclusion
This project investigated potential causes of the temperature asymmetry observed within the SMILE Go vaccine cooler, where the top compartments remained warmer than the bottom ones. We designed a built a temperature monitoring system and then conducted a series of thermal tests on the cooler, investigating convection, potential hotspot areas like the door, uneven ice distribution and ice pack contact. 

Our results showed that the temperature gradient formed very quickly, and although we tested several hypotheses, the gradient still persisted. Some of our design changes did lead to slight reductions in the asymmetry, which suggests the gradient may not be caused by a single dominant mechanism but rather by multiple contributing factors. The door was found not to be a significant cause of the gradient, while the ice pack insert, improved top contact and convection tests showed some slight improvements. The cooler insert showed the most promise, appearing to reduce the difference by up to 2°C. Furthermore, forced air circulation using fans was the only change that reduced the gradient to almost zero. 

Going forward, we recommend conducting further tests to validate and further expand on our findings. Experiments should be repeated to identify any anomalies and ideally, should be conducted in a controlled environment. Additionally, it would be valuable to investigate whether the temperature within a vaccine vial (in a compartment) is the same as the measured compartment temperature.

Based on our findings, the insulation of the cooler should be improved in order to get the compartments within the acceptable temperature range. This improved insulation may also help reduce the asymmetry. Additionally, active low-power systems such as the fan appear viable and should be considered as a potential solution.

Overall, the project assessed a range of potential causes of the temperature gradient seen across the vaccine carousel, but did not identify a single primary cause. Nevertheless, it has provided valuable initial insight into the problem from which further investigation can be continued.
